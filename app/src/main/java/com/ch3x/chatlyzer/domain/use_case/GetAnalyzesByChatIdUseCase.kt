...
class GetAnalyzesByChatIdUseCase @Inject constructor(
    private val analysisRepository: AnalysisRepository
) {
    operator fun invoke(chatId: String): Flow<Resource<List<Analysis>>> = flow {
        try {
            emit(Resource.Loading())
            val result = analysisRepository.fetchAnalysisByChatId(chatId)
